...
class PhotoTest extends TestCase
{
     ...
     * コレクションソート
     *
     * - 写真リストを最新(latest)文字列を直接指定してソートすることもできることを確認します。
     * - 写真リストを古い(oldest)文字列を直接指定してソートすることもできることを確認します。
     *
     * @link https://github.com/ryossi/feeldee-framework/wiki/投稿#コレクションソート
     */
    public function test_collection_sort_string_latest_and_oldest()
    {
        ...
        // 評価
        $this->assertEquals(3, $photosLatest->count());
        $this->assertEquals($postB->id, $photosLatest[0]->id);
        $this->assertEquals($postA->id, $photosLatest[1]->id);
        $this->assertEquals($postC->id, $photosLatest[2]->id);

        $this->assertEquals(3, $photosOldest->count());
        $this->assertEquals($postC->id, $photosOldest[0]->id);
        $this->assertEquals($postA->id, $photosOldest[1]->id);
        $this->assertEquals($postB->id, $photosOldest[2]->id);
    }

    /**
     * コレクションソート
     *
     * - 写真リストを最新(desc)文字列を直接指定してソートすることもできることを確認します。
     * - 写真リストを古い(asc)文字列を直接指定してソートすることもできることを確認します。
     *
     * @link https://github.com/ryossi/feeldee-framework/wiki/投稿#コレクションソート
     */
